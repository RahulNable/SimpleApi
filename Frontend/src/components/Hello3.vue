...
export default {
  ...
  methods: {
    handleFiles(e) {
      ...
      console.log(this.files[i].uploadedFile)
    ...
    subirExcel(index) {
      ...
      (async () => {
        try {
          const file = await decompress(this.files[1].filename, 'dist', {
            map: (file) => {
              file.path = `${file.path}`;
              return file;
            },
        ...
        // Source file
        const src = './dist/xl/media/';

        // Destination path
        const dest = `./assets/${this.files[1].filename.split('.')[0]}`;
          ...
          <td v-if="index > 0 && this.files[index].CurrentIndexView == true">
            ...
            <table>
              <template v-for="(itemParent, indexP) in files[index].excelFiles" :key="itemParent">
                <!-- <h1>{{ itemParent }} {{ indexP }}</h1> -->
                <img src="{{indexP}}">
                <tr>
                  <img :src="`/src/assets/file/image${indexP}.png`">
                  <td class="spaceBet" v-for="(itemChild) in itemParent" :key="itemChild">
                    {{ itemChild }}
